...
import { upsertProcedureRule } from "@/lib/admin/actions";
...
interface ProcedureRuleFormProps {
  rule?: PayerRuleProcedure;
}

export function ProcedureRuleForm({ rule }: ProcedureRuleFormProps) {
  ...
  const [siteRestrictions, setSiteRestrictions] = useState<unknown>(
    rule?.site_of_service_restrictions ?? null
  );
  const [modifiers, setModifiers] = useState<unknown>(
    rule?.modifier_requirements ?? null
  );
  const [freqLimits, setFreqLimits] = useState<unknown>(
    rule?.units_or_frequency_limits ?? null
  );
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const formData = {
      ...
      cpt_code: fd.get("cpt_code") as string,
      procedure_name: fd.get("procedure_name") as string,
      ...
      site_of_service_restrictions: siteRestrictions,
      modifier_requirements: modifiers,
      units_or_frequency_limits: freqLimits,
    ...
    const result: ActionResult = await upsertProcedureRule(
      formData as Parameters<typeof upsertProcedureRule>[0]
    );

    setSaving(false);
    if (result.success) {
      router.push("/admin/rules/procedure");
      router.refresh();
    } else {
    ...
    <form onSubmit={handleSubmit} className="max-w-3xl space-y-8">
      ...
      <fieldset className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <SelectField label="Plan Type" name="plan_type" defaultValue={rule?.plan_type} options={PLAN_TYPES} required />
        </div>
        <SelectField
          label="BCBS Licensee"
          name="bcbs_licensee"
      ...
      <fieldset className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label="CPT Code" name="cpt_code" defaultValue={rule?.cpt_code} required placeholder="e.g. 17311" />
          <Field label="Procedure Name" name="procedure_name" defaultValue={rule?.procedure_name} required />
        </div>
        <Field
          label="ICD-10 Codes"
          name="icd10_codes"
          defaultValue={rule?.icd10_codes?.join(", ") ?? ""}
          placeholder="Comma-separated, e.g. C44.91, L57.0"
        />
      </fieldset>
      ...
      <fieldset className="space-y-6">
        ...
        />
        <JsonTextarea
          name="site_of_service_restrictions"
          label="Site of Service Restrictions"
          schema={siteOfServiceRestrictionsSchema}
          defaultValue={rule?.site_of_service_restrictions}
          onChange={setSiteRestrictions}
        />
        <JsonTextarea
          name="modifier_requirements"
          label="Modifier Requirements"
          schema={modifierRequirementsSchema}
          defaultValue={rule?.modifier_requirements}
          onChange={setModifiers}
        />
        <JsonTextarea
          name="units_or_frequency_limits"
          label="Units or Frequency Limits"
          schema={unitsOrFrequencyLimitsSchema}
          defaultValue={rule?.units_or_frequency_limits}
          onChange={setFreqLimits}
        />
        <JsonTextarea
...
// ---- Helper field components (same as drug-rule-form) ----
...
function Field({
  ...
  return (
    <div>
      ...
      <input id={name} name={name} type={type} defaultValue={defaultValue} placeholder={placeholder} required={required} step={step}
...
function TextareaField({
  label, name, defaultValue, placeholder, required, rows = 3,
}: {
  ...
  return (
    <div>
      <label htmlFor={name} className="block text-sm font-medium text-zinc-700 dark:text-zinc-300">{label}</label>
      <textarea id={name} name={name} defaultValue={defaultValue} placeholder={placeholder} required={required} rows={rows}
        className="mt-1 block w-full rounded-md border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-600 dark:bg-zinc-800" />
    </div>
  );
}
...
function SelectField({
  label, name, defaultValue, options, placeholder, required,
}: {
  label: string; name: string; defaultValue?: string; options: readonly string[]; placeholder?: string; required?: boolean;
}) {
  return (
    <div>
      <label htmlFor={name} className="block text-sm font-medium text-zinc-700 dark:text-zinc-300">{label}</label>
...
function BooleanField({ label, name, defaultValue }: { label: string; name: string; defaultValue: boolean }) {
  return (
    <div>
      <label htmlFor={name} className="block text-sm font-medium text-zinc-700 dark:text-zinc-300">{label}</label>
      <select id={name} name={name} defaultValue={defaultValue ? "true" : "false"}
        className="mt-1 block w-full rounded-md border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-600 dark:bg-zinc-800">
